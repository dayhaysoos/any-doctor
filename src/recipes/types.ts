...
export interface RecipeEvaluationRuntime {
  ...
  identity(file: string, source: string, facts: AnalysisCalls, expression: ExpressionRef, query: IdentityQuery): SemanticResult<{ matches: boolean }>;
...
export interface RecipeDefinition<N extends string, M extends string, Q> {
  ...
  evaluate(
    runtime: RecipeEvaluationRuntime,
    file: string,
    source: string,
    facts: AnalysisCalls,
  ...
  challenges(rule: string, declaration: { name: N; query: Q }): ChallengeFixture[];
}
