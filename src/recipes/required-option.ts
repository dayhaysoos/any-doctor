...
export const requiredOptionRecipe: RecipeDefinition<"required-or-recommended-option", "requiredOrRecommendedOption", RequiredOptionRecipeQuery> = {
  ...
  parse(value) {
    ...
    return call && option ? value as RequiredOptionRecipeQuery : null;
  },
  ...
  evaluate(runtime, file, source, facts, expression, query) {
    ...
    if (!identity.value.matches) return runtime.known("clear", identity.evidence);
    const option = runtime.option(file, source, facts, runtime.ref(subject), query.option);
    if (option.status === "unknown") return runtime.unknownFrom(option);
