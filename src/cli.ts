...
async function cmdGenerate(args: string[]): Promise<void> {
  ...
  const result = executeLoader(doctorPath, "--verify", path.resolve(doctorPath.replace(/\.mjs$/, "") + ".fixtures.mjs")) as unknown as VerifyRunResult;
  let failures = 0;
  for (const c of result.results) {
    if (c.ok) {
      ...
      for (const m of c.missing) console.log("  " + RED + "missing expected finding" + RESET + " " + m.file + ":" + m.line);
      for (const u of c.unexpected) console.log("  " + RED + "unexpected finding" + RESET + " " + u.file + ":" + u.line);
      if (c.error) console.log("  " + RED + "crashed: " + c.error + RESET);
    }
  }
  if (failures > 0) {
