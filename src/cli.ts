...
function doctorSdkDocPath(): string {
...
function authorWorkflowText(): string | null {
...
function exitAfterSurface(outcome: RunOutcome, gate: GateVerdict): number {
  ...
  if ((outcome.broken?.length ?? 0) > 0) {
    for (const b of outcome.broken ?? []) fail(`broken doctor (not scanned): ${b.id}`);
    return 1;
  }
  if (gate.fails) {
...
async function cmdGenerate(args: string[]): Promise<number> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (args[i] === "--stdout") stdoutOnly = true;
  ...
  console.log("");
  if (!stdoutOnly) {
    if (copyToClipboard(prompt)) ok("prompt also copied to clipboard");
    else warn("clipboard unavailable — use the prompt printed above");
  }
...
async function cmdCapabilities(args: string[]): Promise<number> {
  ...
  if (catalog.capabilities.length > 0) {
    ...
    for (const capability of catalog.capabilities) {
      ...
      if (capability.outcomes?.length) console.log(dim("    outcomes: " + capability.outcomes.join(", ")));
      if (capability.limits?.length) console.log(dim("    limits: " + capability.limits.join("; ")));
    }
    console.log("");
  }
...
function validScaffoldSlug(slug: string): boolean {
  return slug.length <= 60 && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug);
}

function doctorScaffold(slug: string): string {
...
function fixturesScaffold(slug: string): string {
...
async function cmdScaffold(args: string[]): Promise<number> {
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  ...
  if (cmd === "help" || cmd === "--help") {
    ...
    if (rest[0] === "capabilities") return await cmdCapabilities(["--help"]);
    usage();
    return 0;
  }
  ...
  try {
    ...
    if (cmd === "scaffold") return await cmdScaffold(rest);
    if (cmd === "capabilities") return await cmdCapabilities(rest);
