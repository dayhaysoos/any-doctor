...
for c in cases: c['source']='const xs=[1,2];\n'+c['source'].replace('[1,2]','xs')
fixtures=[{'name':c['name'],'analysis':'off','seed':{'src/example.ts':c['source']},'expected':[{'rule':c['rule'],'file':'src/example.ts','line':2}] if c['expected'] else []} for c in cases]
