...
function renderReport(input, useColor) {
    ...
    const ordered = [...input.groups].sort((a, b) => {
        const sa = SEVERITY_ORDER.indexOf(a.findings.some(f => f.severity) ? a.findings.find(f => f.severity).severity : a.meta.severity);
        const sb = SEVERITY_ORDER.indexOf(b.findings.some(f => f.severity) ? b.findings.find(f => f.severity).severity : b.meta.severity);
        return sa - sb;
    });
