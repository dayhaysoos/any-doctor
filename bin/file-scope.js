...
export function readAnalysisConfig(root) {
...
export function fileRole(file, config) {
    if (/(^|\/)(dist|build|__generated__|_generated|generated)\/|\.(gen|generated)\.[cm]?[jt]sx?$|\.d\.[cm]?ts$/.test(file) ||
        config.generated.some((p) => matchesPath(file, p)))
        return "generated";
    return isTestPath(file) ? "test" : "authored";
}
export function inventory(root, exts = DEFAULT_EXTS, config = readAnalysisConfig(root)) {
    const files = [];
