...
function opensRegex(source, index, lastCodeChar, lastWord) {
    if (lastCodeChar === "")
        return true; // start of file
    if (REGEX_PRECEDER_CHARS.includes(lastCodeChar))
        return true;
    if (/[A-Za-z0-9_$]/.test(lastCodeChar) && REGEX_PRECEDER_WORDS.has(lastWord))
        return true;
    return false;
}
